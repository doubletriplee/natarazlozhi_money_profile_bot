...
def create_web_app(
    ...
    async def privacy(_: web.Request) -> web.Response:
        ...
        payment_data = "Email и платёжные данные в текущем сценарии бота не запрашиваются."
        providers = "Telegram используется как платформа общения, хостинг — для работы приложения."
        body = f"""
...
<h2>Права пользователя</h2><p>Удалить имя, исходные данные и результаты можно
...
правовые основания, трансграничную передачу и сроки хранения.</p>"""
    ...
    async def terms(_: web.Request) -> web.Response:
        operator = html.escape(settings.operator_name or "[ФИО оператора необходимо заполнить]")
        inn = html.escape(settings.operator_inn or "[ИНН необходимо заполнить]")
        payment_terms = """<h2>Стоимость</h2><p>Определение денежного аватара в боте
предоставляется бесплатно. Полный разбор денежной сферы предлагается отдельно за 990₽;
условия заказа согласовываются в личном чате с исполнителем до оплаты.</p>"""
        refund_terms = (
