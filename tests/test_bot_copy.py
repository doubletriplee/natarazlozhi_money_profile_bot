...
EXPECTED_MONEY_STEPS = {
    "Муза": (
        "Напиши одному знакомому с проектом: «Я могу придумать для тебя 3 идеи для контента, "
        "оформления или продукта и прислать их сегодня. Стоимость — ___ ₽. Нужно?»"
    ),
    "Рассказчица": (
        "Вспомни один случай, когда твой совет помог человеку, и расскажи о нём в трёх фразах: "
        "что было, что ты предложила и что изменилось. В конце добавь: «Могу так же разобрать "
        "твою ситуацию за ___ ₽ — напиши мне»."
    ),
    "Мастерица": (
        "Возьми фото или скрин своей лучшей работы и отправь одному человеку: «Могу сделать для "
        "тебя такой результат за ___ ₽ к ___. Хочешь?»"
    ),
    "Навигатор": (
        "Напиши одному человеку: «За 20 минут разберу твою задачу по ___ и дам план из 3 "
        "конкретных шагов. Стоимость — ___ ₽. Проведём?»"
    ),
    "Вдохновительница": (
        "Запиши видео до 30 секунд: назови одну проблему, дай один первый шаг и скажи: «Если "
        "хочешь понять, что делать дальше, проведу короткий разбор за ___ ₽ — напиши “хочу”»."
    ),
    "Хранительница": (
        "Напиши одному человеку, которому ты уже помогала: «Могу раз в неделю помогать тебе с "
        "___, чтобы эта задача больше не копилась. Месяц сопровождения — ___ ₽. Попробуем?»"
    ),
    "Проводница": (
        "Напиши одному человеку: «За 30 минут помогу тебе разобраться, что мешает ___, и выбрать "
        "один понятный следующий шаг. Встреча стоит ___ ₽. Хочешь?»"
    ),
    "Искательница": (
        "Найди в знакомом проекте одну ошибку или упущенную возможность и напиши владельцу: «Я "
        "заметила, что ___. Могу проверить ещё 5 таких точек и дать список решений за ___ ₽. "
        "Нужно?»"
    ),
    "Создательница": (
        "Отправь одному потенциальному покупателю: «Хочу сделать ___, чтобы помочь с ___. Готова "
        "собрать первую версию за ___ ₽. Тебе это нужно?»"
    ),
    "Эстетка": (
        "Возьми готовую работу из галереи и выложи её без нового оформления с подписью: «Сделаю "
        "для тебя ___ в таком стиле за ___ ₽. Если хочешь — напиши мне»."
    ),
}
...
def test_money_steps_match_approved_copy() -> None:
    assert EXPECTED_MONEY_STEPS.keys() == AVATAR_PRESENTATIONS.keys()
    for avatar_name, expected in EXPECTED_MONEY_STEPS.items():
        assert AVATAR_PRESENTATIONS[avatar_name].caption.rsplit("\n", maxsplit=1)[1] == expected
